...
/**
 * This file was copied verbatum from
 * org.junit.internal.runners.statements.FailOnTimeout.
 * 
 * Credit for the code in package is given to Nelson Llewellyn.
 */
public class TestTimeout extends Statement {
	private final Statement fOriginalStatement;
	private final TimeUnit fTimeUnit;
	private final long fTimeout;

	public TestTimeout(Statement originalStatement, long millis) {
		this(originalStatement, millis, TimeUnit.MILLISECONDS);
	}

	public TestTimeout(Statement originalStatement, long timeout, TimeUnit unit) {
	...
	private static class StatementThread extends Thread {
		/**
		 * This is final variable because the statement is set once. Final makes
		 * sure that the statement is immediately visible in #run() (other than
		 * current thread) after constructor finished.
		 */
		private final Statement fStatement;

		/**
		 * These two variables are volatile to make sure that the Thread calling
		 * #evaluate() can immediately read their values set by this thread.
		 * */
